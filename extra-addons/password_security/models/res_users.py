...
class ResUsers(models.Model):
    ...
    def password_match_message(self):
        ...
        if company_id.password_lower:
            message.append('{} minuscule, \r'.format(company_id.password_lower))
        if company_id.password_upper:
            message.append('{} majuscule, \r'.format(company_id.password_upper))
        if company_id.password_numeric:
            message.append('{} chiffre, \r'.format(company_id.password_numeric))
        if company_id.password_special:
            message.append('{} caractère special'.format(company_id.password_special))
        if message:
            message = [_('Doit contenir :')] + message
        if company_id.password_length:
            message = ['Le mote de passe doit faire {} caractères ou plus.'.format(company_id.password_length)] + message
